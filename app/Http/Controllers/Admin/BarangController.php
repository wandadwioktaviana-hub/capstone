...
class BarangController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('img', function ($row) {
                    ...
                    $default = url('/assets/default/barang/image.png');
                    $imgUrl = $default;
                    if (!empty($row->barang_gambar) && $row->barang_gambar != 'image.png' && Storage::disk('public')->exists('barang/'.$row->barang_gambar)) {
                        $imgUrl = asset('storage/barang/'.$row->barang_gambar);
                    }
                    $img = '<a data-bs-effect="effect-super-scaled" data-bs-toggle="modal" href="#Gmodaldemo8" onclick=gambar(' . json_encode($array) . ')><span class="avatar avatar-lg cover-image" style="background: url(&quot;' . $imgUrl . '&quot;) center center;"></span></a>';
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('img', function ($row) {
                    ...
                    $imgUrl = $default;
                    if (!empty($row->barang_gambar) && $row->barang_gambar != 'image.png' && Storage::disk('public')->exists('barang/'.$row->barang_gambar)) {
                        $imgUrl = asset('barang/'.$row->barang_gambar);
                    }
                    $img = '<span class="avatar avatar-lg cover-image" style="background: url(&quot;' . $imgUrl . '&quot;) center center;"></span>';
    ...
    public function proses_ubah(Request $request, BarangModel $barang)
    {
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($barang->barang_gambar && $barang->barang_gambar != 'image.png' && Storage::disk('public')->exists('barang/'.$barang->barang_gambar)) {
                Storage::disk('public')->delete('barang/' . $barang->barang_gambar);
            }

            //prepare update data with new image
            $oldKode = $barang->barang_kode;
            $newKode = $request->kode;
            $updateData = [
                'barang_gambar'  => $newImageName,
                'jenisbarang_id' => $request->jenisbarang,
                'satuan_id' => $request->satuan,
                'merk_id' => $request->merk,
                'barang_kode' => $newKode,
            ...
            DB::transaction(function() use ($oldKode, $newKode, $updateData, $barang) {
                DB::table('tbl_barangmasuk')->where('barang_kode', $oldKode)->update(['barang_kode' => $newKode]);
                DB::table('tbl_barangkeluar')->where('barang_kode', $oldKode)->update(['barang_kode' => $newKode]);
